...
from attentions import wa_module
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1, norm_layer=None, wavename='haar'):
        ...
        if(stride == 1):
            ...
            self.conv2 = nn.Sequential(wa_module(wavename=wavename),
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1, norm_layer=None, wavename='haar'):
        ...
        if(stride == 1):
            ...
            self.conv3 = nn.Sequential(wa_module(wavename=wavename),
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        if(pool_only):
            self.conv1 = nn.Conv2d(
                3, planes[0], kernel_size=7, stride=2, padding=1, bias=False)
        ...
        if(pool_only):
            ...
            self.maxpool = nn.Sequential(
                *[Downsample(wavename=wavename), Downsample(wavename=wavename)])

        self.layer1 = self._make_layer(
            block, planes[0], layers[0], groups=groups, norm_layer=norm_layer)
        self.layer2 = self._make_layer(
            block, planes[1], layers[1], stride=2, groups=groups, norm_layer=norm_layer, wavename=wavename)
        self.layer3 = self._make_layer(
            block, planes[2], layers[2], stride=2, groups=groups, norm_layer=norm_layer, wavename=wavename)
        self.layer4 = self._make_layer(
            block, planes[3], layers[3], stride=2, groups=groups, norm_layer=norm_layer, wavename=wavename)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, groups=1, norm_layer=None, wavename='haar'):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            ...
            # downsample = [Downsample(wavename=wavename), ] if(
            #     stride == 2) else []
        ...
        layers.append(block(self.inplanes, planes, stride,
                            downsample, groups, norm_layer, wavename=wavename))
        self.inplanes = planes * block.expansion
        for _ in range(1, blocks):
            layers.append(block(self.inplanes, planes, groups=groups,
                                norm_layer=norm_layer, wavename=wavename))
